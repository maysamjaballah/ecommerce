          ...
          <div className="products-grid">
            {products.map(product => (
              <ProductCard key={product.id} product={product} user={user}
                adding={adding} success={success} onAdd={handleAddToCart} navigate={navigate} />
            ))}
          </div>
        </div>
      )}
...
function ProductCard({ product, user, adding, success, onAdd, navigate }) {
  ...
  return (
    <div className={`product-card ${product.stock === 0 && !product.variants?.length ? 'out-of-stock' : ''}`}>
      <div className="product-content" onClick={handleCardClick} style={{ cursor: 'pointer' }}>
